...
def encontrar_horarios_disponibles_en_rango(fecha, hora_inicio_rango, hora_fin_rango, reservas_existentes, duracion_horas=1.5):
    """Encuentra horarios disponibles considerando la duración de la reunión."""
    ...
    while tiempo_actual + duracion_minutos <= fin_rango_min:
        inicio_propuesto = minutes_to_time(tiempo_actual)
        fin_propuesto = minutes_to_time(tiempo_actual + duracion_minutos)
        disponible = True
        ...
        tiempo_actual += 30  # Incremento de 30 minutos entre bloques de búsqueda
    return horarios_disponibles

def reubicar_reserva(reserva_a_reubicar, reservas_fijas, fecha, duracion_horas=1.5):
    ...
    duracion_horas_reserva = duracion_horas
...
def procesar_reserva_con_rango_y_prioridad(nueva_reserva, reservas_existentes):
    ...
    # Usar la duración de la nueva reserva (que se pasa desde la interfaz)
    duracion_horas = nueva_reserva.get('duracion_horas', 1.5)
    reservas_temporales = [r.copy() for r in reservas_existentes]
    horarios_disponibles = encontrar_horarios_disponibles_en_rango(
        fecha, hora_inicio_rango, hora_fin_rango, reservas_temporales, duracion_horas
    )
    ...
    if not conflictos_en_rango:
        todos_horarios_dia = encontrar_horarios_disponibles_en_rango(
            fecha, HORA_INICIO_DIA, HORA_FIN_DIA, reservas_temporales, duracion_horas
        )
        horarios_en_rango = []
    ...
    horarios_con_fijas = encontrar_horarios_disponibles_en_rango(
        fecha, HORA_INICIO_DIA, HORA_FIN_DIA, todas_reservas_fijas, duracion_horas
    )
    horarios_validos = []
    ...
    for reserva_desplazar in reservas_a_desplazar:
        ...
        duracion_reubicar = reserva_desplazar.get('duracion_horas', 1.5)
...
with tab2:
    ...
    with col2:
        ...
        # --- AÑADIR SELECCIÓN DE DURACIÓN ---
        duracion_horas = st.selectbox(
            "Duración de la reunión *",
            options=[1, 2, 3],
            index=0, # Por defecto 1 hora
            key="duracion",
            help="Seleccione la duración estimada de la reunión"
        )
        st.info(f"💡 El sistema asignará un horario de {duracion_horas} hora(s) dentro de su rango preferido")
    ...
    if hora_inicio_rango >= hora_fin_rango:
        ...
        rango_valido = False
    elif time_to_minutes(hora_inicio_rango) + duracion_minutos > time_to_minutes(HORA_FIN_DIA):
         st.error(f"❌ El rango horario debe permitir una reunión de {duracion_horas} hora(s) dentro del horario laboral ({HORA_INICIO_DIA.strftime('%H:%M')} - {HORA_FIN_DIA.strftime('%H:%M')})")
         rango_valido = False
    elif time_to_minutes(hora_inicio_rango) + duracion_minutos > time_to_minutes(hora_fin_rango):
        st.error(f"❌ El rango horario debe permitir una reunión de {duracion_horas} hora(s)")
        rango_valido = False
        ...
        if st.button("✅ Confirmar Reserva", type="primary", use_container_width=True):
            ...
            if errores:
                ...
                nueva_reserva = {
                    ...
                    "duracion_horas": duracion_horas # Añadir duración temporalmente para la lógica
                }
                ...
                if exito:
                    ...
                    if record_id:
                        ...
                        with col_d1:
                            ...
                            st.write(f"**⏱️ Duración:** {duracion_horas} hora(s) (asignada temporalmente)")
...
with tab3:
    ...
    with view_tab2: # Nueva pestaña para la vista de calendario
        ...
        # --- Cálculo de fechas específicas para el calendario ---
        # Usamos la hora local para determinar el mes actual
        hoy_local = obtener_hora_local().date()
        # Definimos el rango para mostrar todo el mes actual
        # Inicio del mes actual
        fecha_inicio_calendario = hoy_local.replace(day=1)
        # Fin del mes actual (inicio del mes siguiente - 1 día)
        if hoy_local.month == 12:
            fecha_fin_calendario = hoy_local.replace(year=hoy_local.year + 1, month=1, day=1) - timedelta(days=1)
        else:
            fecha_fin_calendario = hoy_local.replace(month=hoy_local.month + 1, day=1) - timedelta(days=1)
        # --- Fin cálculo de fechas específicas ---
        # Filtrar reservas para el calendario basado en el mes actual calculado,
        # y en el filtro de criterio (si se aplica).
        # Importante: No usamos fecha_desde y fecha_hasta aquí.
        eventos_calendario = []
        for reserva in st.session_state.reservas:
            if 'fecha' not in reserva:
                continue
            try:
                ...
                if not (fecha_inicio_calendario <= fecha_reserva <= fecha_fin_calendario):
                    continue # Solo incluimos reservas del mes actual
                # Aplicar filtro de criterio si se ha seleccionado alguno
                if filtro_criterio and reserva['criterio'] not in filtro_criterio:
                    continue
                ...
                # Usar la duración temporal para el calendario si está presente, o 1.5 por defecto
                duracion_horas = reserva.get('duracion_horas', 1.5)
                horario_texto = f"{hora_inicio} - {hora_fin} ({duracion_horas}h)"
                ...
                evento = {
                    ...
                    "description": f"Criterio: {reserva['criterio']}\nHorario: {horario_texto}", # Unida en una sola línea con \n
                    "color": color_evento
                }
        ...
        # Mostrar un mensaje con el número de eventos encontrados
        st.info(f"ℹ️ Se encontraron {len(eventos_calendario)} eventos para mostrar en el calendario del mes actual ({fecha_inicio_calendario.strftime('%B %Y')}).")
        # --- FIN VERIFICACIÓN DE DEPURACIÓN ---
        # Configuración del calendario
        calendar_options = {
            "editable": "false", # Hacerlo no editable
            "navLinks": "true",  # Permitir navegación
            "resources": "false", # No usar recursos
            "initialDate": fecha_inicio_calendario.isoformat(), # <-- Usar inicio del mes actual calculado como punto de inicio
        ...
        # Añadimos una key basada en el número de eventos y el mes/año para forzar la reconstrucción si cambian
        # Esto puede ayudar si Streamlit intenta reutilizar una instancia anterior del componente
        calendar_key = f"calendar_{fecha_inicio_calendario.year}_{fecha_inicio_calendario.month}_{len(eventos_calendario)}"
        calendar(
            ...
            """,
            key=calendar_key # <-- Añadida la key
        )
...
with st.sidebar:
    ...
    with st.expander("🕐 Horarios"):
        ...
        - **Duración:** 1, 2 o 3 horas por reunión (configurable al reservar)
